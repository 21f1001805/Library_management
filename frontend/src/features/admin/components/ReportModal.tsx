...
export type ReportKey = 'revenueByPlan' | 'profitAndLoss' | 'expenseBreakdown' | 'membershipGrowth';
        ...
        <div key={item.label} className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium text-foreground">
              ...
              {t('admin.reports.paymentsCount', { count: item.count })}
            </span>
          </div>
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-2">
        {report.months.map((month) => (
          <div
            key={month.month}
            className="flex flex-col gap-1 rounded-md bg-secondary/50 p-2.5 text-sm"
          >
            ...
            <div className="flex items-center justify-between font-medium">
              <span className="text-foreground">
                {t('admin.reports.profitAndLossLabels.netProfit')}
              </span>
      ...
      <div className="flex flex-col gap-1 border-t border-border pt-2 text-sm font-semibold">
        <div className="flex items-center justify-between">
          <span className="text-foreground">
            {t('admin.reports.profitAndLossLabels.totalRevenue')}
          </span>
          <span className="text-foreground">{formatCurrency(report.total_revenue)}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-foreground">
            {t('admin.reports.profitAndLossLabels.totalExpenses')}
          </span>
          <span className="text-foreground">{formatCurrency(report.total_expenses)}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-foreground">
            {t('admin.reports.profitAndLossLabels.totalNetProfit')}
          </span>
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="rounded-lg border border-primary/20 bg-primary/5 px-3.5 py-2.5 text-center text-xs text-foreground shadow-xs">
        <span className="font-semibold text-primary">All-Time Historical Scope:</span> Cumulative
        expenditure breakdown across all library operations
      </div>
      <MultiSegmentPie
      ...
      <div className="flex items-center justify-between border-t border-border pt-3 text-sm font-semibold">
        <span className="text-foreground">
          {t('admin.reports.totalHistoricalExpenses', 'Total Historical Expenses')}
        </span>
        <span className="text-foreground">{formatCurrency(report.total)}</span>
      </div>
    </div>
        ...
        <div key={month.month} className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium text-foreground">
              ...
              {' · '}
              {t('admin.reports.membershipGrowthLabels.totalMembers', {
                count: month.total_members,
              })}
            </span>
          </div>
