...
from app.modules.visits import events, repository
...
def _status_from_visit(member_id: str, visit) -> MemberVisitStatusOut:
    """The member's presence state as implied by their latest visit row.

    Shared by get_member_status (which reads that row) and by the check-in/check-out
    publishers (which already hold the row they just wrote). Two copies of this mapping
    would be two chances for the pushed status and the polled status to disagree — and
    disagreeing is exactly the bug a live badge can't afford, since whichever arrived
    last wins on screen.
    """
    if visit is None:
    ...
    is_in_library = visit.checkedOutAt is None
    return MemberVisitStatusOut(
        member_id=member_id,
        is_in_library=is_in_library,
        checked_in_at=visit.checkedInAt if is_in_library else None,
        last_checked_out_at=visit.checkedOutAt if not is_in_library else None,
...
async def check_in_member(recorded_by: User, member_id: str) -> LibraryVisitOut:
    ...
    # to learn that happened until it asks again. Push it now — publish_status_change
    # swallows Redis failures, so the 30s poll stays the safety net.
    await events.publish_status_change(
        member_id, _status_from_visit(member_id, visit).model_dump_json()
    )
...
async def get_member_status(user: User) -> MemberVisitStatusOut:
    ...
    return _status_from_visit(user.id, latest)
