...
# A real library is never booked to the very last seat — leave some headroom so a walk-in
# member always sees at least one open seat, even during a "peak" hour.
MAX_OCCUPANCY_FRACTION = 0.9
MAX_SEAT_TARGET = math.floor(TOTAL_SEATS * MAX_OCCUPANCY_FRACTION)
...
def _seat_target(hour: int) -> int:
    if hour in PEAK_HOURS:
        ...
        target = DAY_TARGET if hour in DAY_HOURS else NIGHT_TARGET
    return min(target, MAX_SEAT_TARGET)
...
async def _top_up_seat_occupancy(members: list) -> None:
    ...
    for hour in range(24):
        ...
        if needed < 0:
            # A lower cap than a previous run used (e.g. MAX_SEAT_TARGET tightened) can
            # leave today's data over the current target — trim back down rather than
            # leaving stale over-capacity bookings until they roll off with the day.
            excess = RNG.sample(existing, k=-needed)
            for booking in excess:
        ...
        for seat, member in zip(free_seats, candidates[:needed], strict=False):
            try:
                ...
                pass
    print(
        f"Seat occupancy top-up: added {added}, trimmed {trimmed} bookings for "
        f"{today.isoformat()}."
    )
